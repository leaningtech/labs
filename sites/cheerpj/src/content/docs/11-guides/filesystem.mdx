...
To access local files, first import them into the virtual file system (see previous section). To save or download files to the user’s machine, read the virtual file system file in a JavaScript native method (e.g., via `cjFileBlob`) and trigger a browser download after selecting a path with `JFileChooser` if needed.
