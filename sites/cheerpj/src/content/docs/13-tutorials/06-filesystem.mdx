...
Read more about the filesystem in the [CheerpJ documentation](/docs/guides/File-System-support).
...
To use this method, the file you want to copy must already exist in the `/app/` mount point. For example, if you have a file named `notes_tmp.txt` that has been made available in `/app/` (e.g., by being part of your CheerpJ application's resources or mounted via `cheerpjMountFile`), you can then copy it to `/files/`.
