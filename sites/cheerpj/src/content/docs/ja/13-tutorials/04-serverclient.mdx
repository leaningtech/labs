...
CheerpJとTailscaleがどのように連携するかについて詳しくない場合は、[ネットワーキングガイド]をご覧になることをお勧めします。このチュートリアルの完全なテンプレートは[こちら](%CJ_TUTORIALS_BASE%/ServerClientTutorial.zip%CJ_TUTORIALS_QUERY%)からダウンロードできます。
...
> ダウンロード用のZIPは例のソースから生成されています。CheerpJのloader URLが変わった場合は、パッケージスクリプトでZIPを再生成してください。
...
2. アプリケーションのJARファイルを対応するフォルダに貼り付けてください：
  - [サーバーJARをダウンロード](%CJ_TUTORIALS_BASE%/Server.jar%CJ_TUTORIALS_QUERY%)
  - [クライアントJARをダウンロード](%CJ_TUTORIALS_BASE%/Client.jar%CJ_TUTORIALS_QUERY%)
...
プロジェクトテンプレートを[こちら](%CJ_TUTORIALS_BASE%/ServerClientTutorial.zip%CJ_TUTORIALS_QUERY%)からダウンロードするか、このチュートリアルの[GitHubリポジトリ][GitHub repository](https://github.com/leaningtech/cheerpj-meta/tree/main/examples/Server-Client)にアクセスしてください。
