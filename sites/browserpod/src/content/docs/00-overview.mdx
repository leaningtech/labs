...
BrowserPod is ephemeral by design. Each **Pod** runs inside a browser tab and ends when the tab closes or reloads. Because BrowserPod runs inside the browser, each Pod is constrained by the browser’s security model, isolated from the user’s OS, with limited access (via syscalls) to local files or system resources unless you explicitly provide it.
...
- [Tutorials](/docs/getting-started/expressjs)
- [Guides](/docs/guides/hosting)
