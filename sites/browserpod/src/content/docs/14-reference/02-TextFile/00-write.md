...
`write` returns a [Promise] which is resolved when the operation has succeeded. The promise resolves to the number of bytes actually written. On success the seek position is updated. If there is an error during the write, the [Promise] will be rejected.
