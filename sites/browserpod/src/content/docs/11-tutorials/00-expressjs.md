...
We will build it step-by-step, but you can get the end result by running
`npm create browserpod-quickstart` and selecting the "Web Server" template.
...
We will now go through each file, explaining its purpose.
...
This is the main code of the project. A simple hello world web server:
...
This files contains the Vite configuration. See [https://vitejs.dev/config](https://vitejs.dev/config)
...
This is the entry point of our application. It just defines a basic HTML page
with a few elements that we will need to set up BrowserPod.
...
The callbacks gets as an argument a Portal URL: a publicly accessible URL that
you can use to interact with the http server.
We will display it in the iframe that we set up in our `index.html` page.

Then we copy the project files inside the Pod, using the `copyFile` utility function.

Finally, we run some commands inside our Pods: `npm install` to install express.js
