...
This guide will help you set up a basic server using nginx to run your project with BrowserPod. BrowserPod requires [SharedArrayBuffer](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/SharedArrayBuffer), which itself requires the site to be cross-origin isolated. This means that the server must set the appropriate headers.
...
This is a basic nginx configuration that allows BrowserPod to run.
