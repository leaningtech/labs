...
Now you need to setup a simple web application using BrowserPod, you can do so quicky using the following commands:
...
Then put the `hello` file in the `public/` directory, and edit `src/main.js` to look like this:
...
// Add our binary to the filesystem
const f = await pod.createFile("/home/user/hello", "binary");
...
// Run our binary
await pod.run("/home/user/hello", [], {terminal:terminal});
```

You will need to get a free BrowserPod API key from the [BrowserPod console](https://console.browserpod.io) and store it in the `.env` file.

```env
VITE_BP_APIKEY=bp1_your_api_key_here
```
...
And open your browser at localhost port exposed by vite, for example [https://localhost:5173](https://localhost:5173).
You should see something like this:

![Our C program running in BrowserPod](./images/hello-c-bp.png)
...
BrowserCode is [FOSS](https://github.com/leaningtech/browsercode) and it’s intended to be a reference integration of BrowserPod designed to showcase its capabilities. You can fork the repo to build your version, or something else entirely, by adding your own a free BrowserPod API key.

You can generate API keys by logging on the [BrowserPod console](https://github.com/leaningtech/browsercode). From the console you will also be able to test BrowserPod on ready-made sandboxes running many popular Node.js frameworks (Express.js, Next.js, Nuxt, React, Svelte, …) and command line tools.
