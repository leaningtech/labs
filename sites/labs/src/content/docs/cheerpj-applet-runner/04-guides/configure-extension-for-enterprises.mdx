...
As an administrator, you can configure browser extensions for Chrome and Edge using the Group Policy Manager or the Windows Registry.
...
The extension ID is a unique identifier used to differentiate one extension from another.
...
If you are self-hosting a custom build of the Applet Runner, the extension ID will differ.
We either provided you with the extension ID, or you will need to retrieve it yourself.
To obtain the extension ID for a custom build, drag the CRX file onto your browser's Extensions page.
...
Settings must be stored under the following registry path, replacing `<extension-id>` with the extension’s actual ID (see previous [Extension ID](#the-extension-id) section):
