<script>
  ...
  let activeConsole = showTerminalTab ? 'terminal' : (showReplTab ? 'repl' : null);
  ...
  onMount(async () => {
    ...
    try {
      ...
      const { BrowserPod } = await import('https://rt.browserpod.io/0.9.6/browserpod.js');
      pod = await BrowserPod.boot({apiKey:"bp1_74c9fd5dfa63d386d410f9dad99691d4f4a0776bff3b267e72851604d7a98c46"});
  ...
  async function runReadyActions() {
    ...
    for (const action of readyActions) {
      ...
      const targetTerminal = terminal === 'repl'
        ? replTerminal
        : terminal === 'none'
          ? null
          : mainTerminal;
...
>
  <div class="flex flex-col md:flex-row w-full h-full gap-2 md:gap-4 p-2 bg-black/30 backdrop-blur-md border border-white/10 rounded-2xl">
    ...
    {#if !isCompatibleBrowser}
      <div class="absolute inset-0 z-50 flex items-center justify-center backdrop-blur-[2px] rounded-2xl p-4">
        <div class="max-w-md text-center px-6 py-8 bg-black/70 backdrop-blur-md rounded-xl border border-white/15 shadow-lg">
          <div class="p-4 bg-amber-500/20 rounded-full inline-flex mx-auto mb-4">
            <Icon icon="heroicons:exclamation-triangle" class="text-amber-400 w-10 h-10" />
          </div>
          <h3 class="text-xl font-bold text-white mb-3">Incompatible Browser</h3>
          <p class="text-gray-200 mb-4">
            This demo requires a browser supporting <strong class="text-white">Atomics.waitAsync</strong> (Chrome, Edge, Safari) to function properly.
          </p>
    ...
    <div class="flex flex-col gap-2 md:gap-4 w-full md:flex-[2.7] min-w-0 h-[50%] md:h-full">
      {#if showEditor}
      <div
      ...
      >
        ...
        >
          ...
          >
            ...
            <span class="text-emerald-400 truncate max-w-[120px] sm:max-w-none">{selectedFile}</span>
          </div>
        </div>
        <div
        ...
        {#if loading}
          <div class="flex items-center justify-center h-full text-white/70">
            <div class="animate-pulse flex flex-col items-center">
              <Icon icon="heroicons:arrow-path" class="animate-spin h-6 w-6 sm:h-8 sm:w-8 text-emerald-500 mb-2" />
      ...
      >
        ...
        <div class="flex-1 min-h-0 bg-gradient-to-b border border-white/15 rounded-b-xl shadow-2xl relative">
            {#if showTerminalTab}
              <div class={`console w-full h-full rounded-b-xl border-none bg-black terminal-text-smaller pl-2 absolute [&:not(.active)]:invisible ${activeConsole === 'terminal' ? 'active' : ''}`} id="main-console" style="overflow: hidden;"></div>
            {/if}
            {#if showReplTab}
              <div class={`console w-full h-full rounded-b-xl border-none bg-black terminal-text-smaller pl-2 absolute [&:not(.active)]:invisible ${activeConsole === 'repl' ? 'active' : ''}`} id="repl-console" style="overflow: hidden;"></div>
            {/if}
        </div>
      </div>
      {/if}
    ...
    {#if showPreview}
    <!-- Preview Panel -->
    <div class="flex flex-col min-w-0 w-full md:flex-[2.3] h-[50%] md:h-full">
      <div
        class="flex flex-col shadow-2xl h-full overflow-hidden"
      >
        <div
          class="flex items-center justify-between bg-gradient-to-b from-black/50 to-black/20 border border-white/15 rounded-t-xl shadow-2xl px-3 py-1.5"
        >
          ...
          {#if portalURL}
            {#if isMobile}
              ...
              >
                <Icon icon="heroicons:clipboard-document" width="14" height="14" />
                <span class="ml-1">{copied ? "Copied!" : "Copy Portal"}</span>
              </button>
            {:else}
              <button 
              ...
              >
                <Icon icon={showPortalInfo ? "heroicons:eye-slash" : "heroicons:qr-code"} width="14" height="14" />
                <span class="ml-1">{showPortalInfo ? "Hide Portal" : "Show Portal"}</span>
              </button>
            {/if}
          {:else}
            <button 
        ...
        <div class="flex-1 min-h-0 bg-gradient-to-b from-black/50 to-black/20 border border-white/15 rounded-b-xl shadow-2xl border-t-0 relative">
        ...
        <div class="fade-element absolute top-0 left-0 w-full h-full flex flex-col items-center justify-center bg-black/50 backdrop-blur-xs p-4">
          <h2 class="text-white text-2xl font-bold mb-6 tracking-wide drop-shadow-lg text-center">
            Scan this Portal
          </h2>
          <div class="qr-code bg-white p-2 rounded-lg mb-4">
            <canvas bind:this={qrCodeCanvas} width="150" height="150"></canvas>
          </div>
          <div class="portal-url bg-black/60 p-3 rounded-lg w-full max-w-sm">
            <p class="text-white text-xs sm:text-sm break-all text-center mb-2">
              {portalURL}
            </p>
          ...
          onclick={async () => {
            ...
            copiedTimeout = setTimeout(() => copied = false, 1200);
              ...
              >
          <Icon icon="heroicons:clipboard-document" class="mr-1" width="14" height="14" />
          {#if copied}
            Copied!
          {:else}
            Copy URL
          ...
          onclick={togglePortalInfo}
              >
          <Icon icon="heroicons:x-mark" class="mr-1" width="14" height="14" />
          Hide
              </button>
            </div>
          </div>
        ...
        <div class="fade-element absolute top-0 left-0 w-full h-full flex flex-col items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <h2 class="text-white text-xl font-bold mb-4 tracking-wide drop-shadow-lg text-center">
            Scan this Portal
          </h2>
          <div class="qr-code bg-white p-2 rounded-lg mb-3">
            <canvas bind:this={qrCodeCanvas} width="150" height="150"></canvas>
          </div>
          <div class="portal-url bg-black/60 p-3 rounded-lg w-full max-w-xs">
            <p class="text-white text-xs break-all text-center mb-2">
              {portalURL}
            </p>
            <div class="flex space-x-2 justify-center">
              <button 
      ...
      {#if isMobile && portalURL && !showMobilePortal}
        <div class="mt-2 flex justify-center">
          <button 
            ...
            onclick={toggleMobilePortal}
          >
            <Icon icon="heroicons:qr-code" class="mr-2" width="16" height="16" />
            Show Portal QR Code
          </button>
        </div>
      {/if}
    ...
    {#if !isCompatibleBrowser}
      <div class="absolute inset-0 backdrop-blur-[2px] bg-gradient-to-br from-emerald-950/40 to-slate-950/50 pointer-events-none z-20"></div>
    {/if}
  </div>
</div>
