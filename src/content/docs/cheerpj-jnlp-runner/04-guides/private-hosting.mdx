...
> You can also use the WebStore's download URL, but you won't be self-hosting the extension and it will be retrieved from the WebStore for all your users. This is a limitation for IT infrastructures where the organisation devices do not have connection permissions to the wider internet. For this reason we recommend the self-hosting method.

## The extension ID

The extension ID is a unique identifier used by Google. You can easily retrieve it if you have the extension installed in the traditional way from the WebStore:

1.  Go to the extensions menu in Chrome (puzzle icon) and click on _Manage extensions_
2.  All your installed extensions will show. Now turn on the slider for _Developer Mode_.
3.  You will see the ID displayed for all your installed extensions. Copy the JNLP Runner's ID and reserve for later.
...
`cciilfdpmibiokimppmdpopaolambagi;https://some.download.url.file.crx`

> [!warning] Hypothetical policy string
> The example above contains hypothetical ID and URL, please do not use this policy string.

## Policy templates

You will need to download and install the Google administrative templates. You can do this by downloading the templates file and following the steps as stated in this [guide by google](https://support.google.com/chrome/a/answer/187202?hl=en)

## Creating a new group policy

Now you have everything you need to create a new GPO, please follow these final steps:

1. Open the Group Policy Management interface
2. Go to your desired domain and git the right click to display the options menu.
...
## Further group policy setup

Remember you can always configure other settings for this Group Policy that are appropriate for your group of users and the extension.
...
In this window you can configure all the settings relevant for your organisation when using the JNLP runner extension. Remember to use the extension ID for those settings that you want to directly apply to the JNLP Runner extension. This is done by editing the JSON string shown in the interface using the format:
