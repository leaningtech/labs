---
title: Edge Installation
description: Install CheerpJ JNLP Runner for Edge
---

This tutorial will take you step by step on how to install the CheerpJ JNLP Runner extension for Edge.

## Step 1

Please start by visiting the [Edge's add-on JNLP Runner page](https://microsoftedge.microsoft.com/addons/detail/cheerpj-jnlp-runner/dpleickbpchmkpkmicanfolmianepfhe). Once there, please click on the button _**Get**_ to start the installation.

<div class="mx-24">![](/cheerpj3/assets/jnlp-screenshots/edge_get.png)</div>

It will prompt a dialogue showing that the extension will be able to manage your downloads, then click on _**add extension**._

<div class="mx-24">
	![](/cheerpj3/assets/jnlp-screenshots/edge_dialogue.png)
</div>

> [!info]About downloads permission
> CheerpJ JNLP Runner needs to see your downloads. This is because the classic way to run JNLP applications is by triggering the download of its corresponding JNLP file. When CheerpJ JNLP Runner detects the download of a JNLP file it will start working with your Java application.

## Step 2

Once the extension is installed you will the small logo to the right of the browsing bar. If the logo is not there, click on the puzzle piece icon to see all your installed extensions, it should appear right there. Here you can click on the extension menu (three dots) and select _*Show in Toolbar*_ .

<div class="mx-24">![](/cheerpj3/assets/jnlp-screenshots/edge_toolbar.png)</div>

## The end

This tutorial has come to an end and your CheerpJ JNLP extension must be already installed on your Edge browser. The next step is to validate your license, please visit the [activate your license](/cheerpj-jnlprunner/guides/licensing) tutorial.
