...
2.  Create two ephemeral auth keys [here](https://login.tailscale.com/admin/settings/keys) and reserve, you will be using these keys to authenticate the Java applications into your Tailscale network later.

> [!tip] Ephemeral keys
> Remember to create another auth key every time you reload a page. Ephemeral keys are for one-time use only. Remember to delete machines from your Tailscale network as needed.
...
1. Create an HTML file inside each subfolder, let's name it `client.html` and `server.html`
...
4. Now open a browser tab and visit the address you are serving. It should be something similar to `http://127.0.0.1:8080/server.html`. You will see the CheerpJ loading animation for a brief moment and your application should show.
5. Open the [Tailscale Dashboard](https://login.tailscale.com/admin/machines) where you should see the connected users/machines. Identify the one that corresponds to the Server application and reserve its API address. You will need it later.
	...
	<body>
		<script>
			(async function () {
				...
				cheerpjCreateDisplay(800, 600);
				await cheerpjRunJar("/app/Client.jar", ["ServerIPAddressHere"]);
			})();
		</script>
	</body>
</html>
...
3. Copy the Server's IP address from the Tailscale dashboard and paste it as a string replacing _"ServerIPAddressHere"_. For example, the second parameter could look something like this:`["100.80.80.33"]`.
4. Now you are ready to serve this page and run the Client application in the browser. Simply run a HTTP server **inside** the `Client` directory with a different port than the one used for the Server app. For example:
...
5. Now open a browser tab and visit the address you are serving. It should be something similar to `http://127.0.0.1:8081/client.html`. You will see the CheerpJ loading animation for a brief moment and your application should show.
...
<table>
	<tr>
	<th>
		<img src="/cheerpj2/assets/browser-server.png" ></img>
	</th>
	<th>
		<img src="/cheerpj2/assets/browser-client.png" ></img>
	</th>
	</tr>

</table>

You can download the full template of this tutorial [here](/cheerpj3/examples/ServerClientTutorial.zip).
