...
2.  A second script calling [`cheerpjInit({enablePreciseAppletArchives:true})`] to initialize CheerpJ's runtime environment.
...
> [!tip] Avoiding conflicts with native Java plugins
...
[`cheerpjInit({enablePreciseAppletArchives:true})`]: /cheerpj2/reference/Runtime-API#enablepreciseappletarchives
