---
title: "Introducing TypeScript integration in Cheerp"
...
that adds *static type checking* to JavaScript, allowing us to seamlessly share
...
want `pop` to return `double`, *not* `double*`. But when passing in a
...
references to JavaScript objects. There is one downside, while you *can*
...
sometimes useful to say that a value can have *any* type. What do we call the
...
By instead using a const reference union type for the constructor, we *can*
...
can be constructed from other functions with a *covariant* return type and an
equal or smaller number of *contravariant* arguments.
