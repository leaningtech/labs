...
*await* cheerpjRunJar("/app/my\_application\_archive.jar");  
\`\`\`
...
The extension has been wildly popular, having been used by hundreds of thousands of users all over the world. Although they are extremely convenient for end-users and during testing, it is \_not\_ necessary to use extensions for applets or Web Start, CheerpJ can be directly integrated in existing or new Web pages if preferred.
...
LWJGL in particular is interesting, since it is only \_superficially\_ Java. Most of its value comes from JNI methods which provide direct access to each and every method exposed by OpenGL. These methods are written in C and automatically generated by the LWJGL build system from a declarative representation of the OpenGL API.
